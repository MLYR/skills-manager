...
pub fn validate_ai_analysis_result_v1(
    ...
    result.one_line = normalize_string(&result.one_line, MAX_ONE_LINE_CHARS)?;
    result.what_it_does = normalize_string(&result.what_it_does, MAX_WHAT_IT_DOES_CHARS)?;
    result.when_to_use = normalize_list(&result.when_to_use, MAX_STANDARD_ARRAY_ITEMS)?;
    result.how_to_use = normalize_list(&result.how_to_use, MAX_STANDARD_ARRAY_ITEMS)?;
    result.example_prompts = normalize_list(&result.example_prompts, MAX_EXAMPLE_PROMPTS)?;
    result.requirements = normalize_list(&result.requirements, MAX_STANDARD_ARRAY_ITEMS)?;
    result.not_for = normalize_list(&result.not_for, MAX_STANDARD_ARRAY_ITEMS)?;
    result.warnings = normalize_list(&result.warnings, MAX_STANDARD_ARRAY_ITEMS)?;
...
fn normalize_list(values: &[String], maximum_items: usize) -> Result<Vec<String>, AiCommandError> {
    ...
    values
        .iter()
        .map(|value| normalize_string(value, MAX_ARRAY_ITEM_CHARS))
        .collect()
}

fn normalize_string(value: &str, maximum_chars: usize) -> Result<String, AiCommandError> {
    if value.contains('\0') {
        return Err(schema_validation_error());
    }
...
mod tests {
    ...
    fn rejects_empty_nul_and_out_of_bounds_values() {
        ...
        assert_eq!(
            validate(one_line)
                .expect_err("one_line limit must fail")
                .code,
            AiErrorCode::SchemaValidation
        );
        ...
        assert_eq!(
            validate(examples)
                .expect_err("example array limit must fail")
                .code,
            AiErrorCode::SchemaValidation
        );
